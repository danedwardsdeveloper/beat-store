...
export default async function BeatsList() {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-0 md:gap-4">
      {Array.from({ length: 10 }, (v, i) => beats[i % 2]).map((beat, index) => (
        <BeatCard key={index} beat={beat} eagerLoading={index < 10} />
      ))}
    </div>
  )
}
